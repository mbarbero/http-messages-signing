...
public abstract class HttpMessageSignatureVerifier {
	...
	public boolean verify(HttpRequest signedRequest) throws HttpMessageSignatureVerificationException, GeneralSecurityException {
		try {
			HttpMessageSignatureHeaderElements signatureHeader = HttpMessageSignatureHeaderElements.fromHeaderValuesList(signedRequest.headerValues(HttpMessageSigner.HEADER_SIGNATURE));
			String signingString = SigningStringBuilder.forHeaders(signatureHeader.signedHeaders()).signingString(signedRequest);
			...
			throw new HttpMessageSignatureVerificationException("Unable to verify request '"+signedRequest.toString()+"'", e);
		}
	}
